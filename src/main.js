...
IsoBlock.makeFigure = function (options) {
  ...
  var shouldSortBlocks =
    options.sortBlocks == undefined ? true : options.sortBlocks;
  ...
  var scale =
    (options.scale && options.scale(canvas.width, canvas.height)) ||
    canvas.height / 8;
  var origin = (options.origin &&
    options.origin(canvas.width, canvas.height)) || {
    x: canvas.width / 2,
    y: canvas.height,
  };
  ...
  if (!axisLen) {
    ...
    axisLen =
      Math.floor((canvas.width - origin.x) / scale / Math.cos(Math.PI / 6)) -
      0.5;
  }
